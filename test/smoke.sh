...
# ---------------------------------------------------------------------------
# WorktreeRemove hook: a side-effect that rescues node_modules to the pool.
# (nmpool registers NO WorktreeCreate hook — that event replaces git's native
#  worktree creation and exists for non-git VCS; Claude creates git worktrees.)
# ---------------------------------------------------------------------------
...
echo "== case 11: WorktreeRemove hook returns a worktree's node_modules to the pool =="
WT="$TMP/hookwt/feat-a"
git -C "$HREPO" worktree add -q "$WT" -b feat-a   # Claude would do this natively
python3 "$NMPOOL" install --worktree "$WT" >/dev/null
[ -d "$WT/node_modules" ]; check "precondition: worktree has node_modules" $?
